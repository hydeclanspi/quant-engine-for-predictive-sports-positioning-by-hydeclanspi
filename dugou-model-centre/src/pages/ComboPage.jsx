...
// 故此值即播放前留白：原 850ms 基线净 +730ms（延后 1.14s，先后提前 0.17/0.24s）= 1580ms（动画内部时长不变）。
const PORTFOLIO_PEEK_DELAY_MS = 1580 // 结果揭晓 → 开始展卷 的等待
const PORTFOLIO_PEEK_UNFURL_MS = 960 // 展卷动画时长（与 qiUnfurlCalm 对齐）
const PORTFOLIO_PEEK_DAWN_MS = 1250 // 破晓柔光时长（与 qiDawn 对齐）后撤下
const PORTFOLIO_PEEK_HOLD_MS = 1055 // 展卷完成 → 开始收回 的停留（停留累计收紧 −0.35/−0.43/−0.21/−0.125s；展卷/收锋的动作速度始终不变）
const PORTFOLIO_PEEK_REFOLD_MS = 860 // 收锋（倒卷折叠）时长（与 is-peek-refold 对齐）
// 【最优】折回完成 → 明细列表「第一条」开始「秀一下」的间隔（接力第二段揭幕）。
const COMBO_DETAIL_PEEK_GAP_MS = 550
...
const parseDirectedMatrixRequirements = (requirements) => {
  ...
  requirements.forEach((item) => {
    ...
    if (!fromKey || !toKey || fromKey === toKey) return
    const requiredSurvives = Boolean(item?.requiredSurvives)
    const key = `${fromKey}|${toKey}`
    if (seen.has(key)) return
    seen.add(key)
    parsed.push({ key, fromKey, toKey, requiredSurvives })
  })
  return parsed
}
...
const buildDirectedMatrixRequirementsFromPortfolio = (baseCombos, overrides = null) => {
  ...
  safeBaseCombos.forEach((combo) => {
    getSubsetMatchKeys(combo?.subset || []).forEach((key) => {
      if (key) matchKeys.add(String(key))
    })
  })
  const keys = [...matchKeys]
  const requirements = []
  const strictTruePairs = new Set()
  const explicitDecouplePairs = new Set()
  for (let i = 0; i < keys.length; i += 1) {
    for (let j = 0; j < keys.length; j += 1) {
      ...
      const intent = overrideMap[pairKey]
      if (intent === 'release') continue
      if (intent === 'decouple') explicitDecouplePairs.add(pairKey)
      const baseSurvives = safeBaseCombos.some((combo) =>
        comboSatisfiesDirectedConstraint(combo, { fromKey, toKey }),
      )
      const requiredSurvives = intent === 'decouple' ? true : Boolean(baseSurvives)
      requirements.push({ fromKey, toKey, requiredSurvives })
      if (requiredSurvives) strictTruePairs.add(pairKey)
    }
  }
  return {
    requirements: parseDirectedMatrixRequirements(requirements),
    strictTruePairs,
    explicitDecouplePairs,
    trackedMatchKeys: keys,
  }
}

const solvePortfolioByDirectedMatrixRequirements = (
  candidateCombos,
  matrixRequirements = [],
  options = null,
) => {
  ...
  const evaluate = () => {
    ...
    for (let rIdx = 0; rIdx < requirements.length; rIdx += 1) {
      ...
      const survives = chosenIdx.some((idx) => supportMatrix[rIdx]?.[idx])
      if (required.requiredSurvives ? !survives : survives) return
    }

    const scoredRow = buildPortfolioAllocationRowFromCombos(
      selected,
      pool,
      coverageTargetKeys,
      hyperparams,
    ...
    if (!scoredRow) return
    feasibleCount += 1
    const utility = Number(scoredRow.utility || 0)
    const coverageRatio = Number(scoredRow.coverageRatio || 0)
    const weightedEv = Number(scoredRow.weightedEv || 0)
    ...
    if (Math.abs(utility - best.utility) <= 1e-9) {
      if (coverageRatio > best.coverageRatio + 1e-9) {
        best = { selected, scoredRow, utility, coverageRatio, weightedEv }
        return
      }
      if (
        Math.abs(coverageRatio - best.coverageRatio) <= 1e-9 &&
        weightedEv > best.weightedEv + 1e-9
      ) {
        best = { selected, scoredRow, utility, coverageRatio, weightedEv }
      }
    }
...
export default function ComboPage({ openModal }) {
  ...
  // 明细列表「第一条」的自动「秀一下」瞬态标记：与【最优】共用同一套展卷/破晓/收锋编排，
  ...
  useEffect(() => {
    ...
    // ─── 接力第二段：【最优】收锋完毕，稍候让「明细列表」第一条同样「秀一下」。
    //     完全复用同一套展卷/破晓/停留/收锋时长，仅换一组瞬态标记与展开集合。
    if (quickPreviewRecommendations.length > 0) {
      const comboIdx = 0
      const comboPeekStartAt = refoldAt + PORTFOLIO_PEEK_REFOLD_MS + COMBO_DETAIL_PEEK_GAP_MS
      // ⑤ 接力起手 → 展卷 + 破晓柔光。
      timers.push(
        window.setTimeout(() => {
          setComboPeekIdx(comboIdx)
          setComboPeekDawnIdx(comboIdx)
          setExpandedComboIdxSet((prev) => {
            if (prev.has(comboIdx)) return prev
            const next = new Set(prev)
            next.add(comboIdx)
            return next
          })
      ...
      timers.push(
        window.setTimeout(() => {
          setComboPeekDawnIdx((cur) => (cur === comboIdx ? null : cur))
        }, comboPeekStartAt + PORTFOLIO_PEEK_DAWN_MS),
      )
      // ⑦ 展卷完成 + 停留 → 倒卷收回。
      const comboRefoldAt = comboPeekStartAt + PORTFOLIO_PEEK_UNFURL_MS + PORTFOLIO_PEEK_HOLD_MS
      timers.push(
        window.setTimeout(() => {
          setExpandedComboIdxSet((prev) => {
            if (!prev.has(comboIdx)) return prev
            const next = new Set(prev)
            next.delete(comboIdx)
            return next
          })
        }, comboRefoldAt),
      )
      ...
      timers.push(
        window.setTimeout(() => {
          setComboPeekIdx((cur) => (cur === comboIdx ? null : cur))
        }, comboRefoldAt + PORTFOLIO_PEEK_REFOLD_MS),
      )
    }
  ...
  const handleFtConfirmRegenerate = (portfolioKeyInput, overrideInput = null) => {
    ...
    const positiveConstraints = parseDirectedDecouplingConstraints(matrixProfile.strictTruePairs)
    const customRow =
    ...
    const customKey = getPortfolioKeyFromAllocationRow(customRow, 'custom-matrix')
    const merged = normalizePortfolioAllocations([
      { ...customRow, portfolioKey: customKey, pinned: true },
      ...autoAllocations
        .filter((row) => getPortfolioKeyFromAllocationRow(row, '') !== customKey)
        .map((row) => (row?.pinned ? { ...row, pinned: false } : row)),
    ].slice(0, 10))
    setPortfolioAllocations(merged)
                      ...
                      <div className="mt-2 rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(249,250,255,0.95) 0%, rgba(255,255,255,0.92) 50%, rgba(245,248,255,0.95) 100%)', backdropFilter: 'blur(16px) saturate(120%)', border: '1px solid rgba(99,102,241,0.08)', boxShadow: '0 4px 20px -4px rgba(99,102,241,0.10), inset 0 1px 0 rgba(255,255,255,0.8)' }}>
                        ...
                        <div className="p-4 space-y-2.5">
                          {alloc.combos.map((combo, cIdx) => {
                            ...
                            const sharpeBadgeToneClass = getSharpeBadgeToneClass(sharpeRaw)
              ...
              {(() => {
                ...
                return quickPreviewRecommendations.map((item, idx) => {
                  ...
                  <div key={item.id}>
                    ...
                    {/* Expandable Detail Panel — 与「最优」行共用 qi-collapse 展卷骨架，故首条可在揭晓后自动「秀一下」。 */}
                    <div
                      className={`qi-collapse${expanded ? ' is-open' : ''}${comboPeekIdx === idx ? ' is-peek-unfurl is-peek-refold' : ''}${comboPeekDawnIdx === idx ? ' is-peek-dawn' : ''}`}
                      inert={expanded ? undefined : ''}
                    >
                      <div className="qi-collapse-inner">
                      <div className="ml-4 mt-1.5 p-3.5 rounded-xl border border-stone-100/80" style={{ background: 'linear-gradient(135deg, rgba(249,250,255,0.92) 0%, rgba(255,255,255,0.88) 100%)' }}>
