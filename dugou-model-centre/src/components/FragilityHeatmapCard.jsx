...
export function FragilityHeatmapCard({ matches = [], expandedPair = null, onSelectPair = null }) {
  ...
  const historicalData = useMemo(() => {
    ...
    return investments
      .filter(inv => inv.is_settled === true || inv.status === 'settled')
      .slice(0, 100) // 限制前100个已结算投资以保持性能
      .map(inv => ({
        matches: Array.isArray(inv.matches)
          ? inv.matches.map(m => ({
              odds: Number(m.odds) || 1,
              result: m.status === 'won' || m.result === true,
            }))
          : [],
        succeeded: inv.status === 'won' || inv.status === 'settled_win',
